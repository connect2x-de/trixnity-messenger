...
class TimelineElementHolderViewModelTest {
    ...
    fun `isFirstInUserSequence » be true when first in a user sequence`() = runTest {
        ...
        timeline(roomServiceMock, roomId) {
            (1..4).forEach { +messageEvent(sender = bobId) { text("World-$it") } }
            // should be ignored
            +MessageEvent(
                content =
                    UnknownEventContent(
        ...
        eventually(100.milliseconds) { cut.isFirstInUserSequence.value shouldBe true }
    }
    ...
    fun `isFirstInUserSequence » false when not first in a user sequence`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        timeline(roomServiceMock, roomId) {
            (1..4).forEach { +messageEvent(sender = aliceId) { text("World-$it") } }
            // should be ignored
            +MessageEvent(
                content =
                    UnknownEventContent(
        ...
        val cut = cut()
        backgroundScope.launch { cut.isFirstInUserSequence.collect() }
        eventually(100.milliseconds) { cut.isFirstInUserSequence.value shouldBe false }
    }
    ...
    fun `showSender » be true when first in a user sequence`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        timeline(roomServiceMock, roomId) {
            +messageEvent(sender = bobId) { text("Hi!") }
            +timelineEvent
        }
        val cut = cut()
        backgroundScope.launch { cut.showSender.collect() }
        eventually(100.milliseconds) { cut.showSender.value shouldBe true }
    }
    ...
    fun `showSender » be true when state event before`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        timeline(roomServiceMock, roomId) {
            +messageEvent(sender = aliceId) { text("Hi!") }
            +stateEvent(sender = aliceId) { content = MemberEventContent(membership = Membership.JOIN) }
            +timelineEvent
        }
        val cut = cut()
        backgroundScope.launch { cut.showSender.collect() }
    ...
    fun `showSender » false when not first in a user sequence`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        every { userServiceMock.getAll(roomId) } returns flowOf(mapOf(aliceId to flowOf(alice)))
        timeline(roomServiceMock, roomId) {
            +messageEvent(sender = aliceId) { text("Hello!") }
            +timelineEvent
        }
        val cut = cut()
        backgroundScope.launch { cut.showSender.collect() }
        eventually(100.milliseconds) { cut.showSender.value shouldBe false }
    }

    @Test
    fun `showSender » be false when we are the sender`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        every { userServiceMock.getAll(roomId) } returns flowOf(mapOf(bobId to flowOf(bob), usId to flowOf(us)))
    ...
    fun `showSender » be false when room is direct`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        every { userServiceMock.getAccountData(DirectEventContent::class, any()) } returns
            flowOf(DirectEventContent(mapOf(aliceId to emptySet())))
        val timeline =
            timeline(roomServiceMock, roomId) {
                +messageEvent(sender = aliceId) { text("Hello!") }
                +timelineEvent
                +messageEvent(sender = usId) { text("Hello!") }
            }
    ...
    fun `isRead » should use latest replacement`() = runTest {
        ...
        every { roomServiceMock.getTimelineEventRelations(roomId, eventId, RelationType.Replace) } returns replacement
        ...
        eventually(100.milliseconds) { cut.isRead.value shouldBe true }
        timeline.addEvents {
            +TimelineEvent(
                event =
                    MessageEvent(
    ...
    fun `showBigGapBefore » be true when first in a user sequence`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        timeline(roomServiceMock, roomId) {
            +messageEvent(sender = bobId, sentAt = Instant.fromEpochMilliseconds(timelineEvent.originTimestamp)) {
                text("Hi!")
            }
            +timelineEvent
        }
        ...
        eventually(100.milliseconds) { cut.showBigGapBefore.value shouldBe true }
    }
    ...
    fun `showBigGapBefore » false when not first in a user sequence`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        timeline(roomServiceMock, roomId) {
            +messageEvent(sender = aliceId, sentAt = Instant.fromEpochMilliseconds(timelineEvent.originTimestamp)) {
                text("Hi!")
            }
            +timelineEvent
        }
        val cut = cut()
        backgroundScope.launch { cut.showBigGapBefore.collect() }
        eventually(100.milliseconds) { cut.showBigGapBefore.value shouldBe false }
    }
    ...
    fun `showUnreadMarker » show the unread marker when event is set`() = runTest {
        ...
        eventually(100.milliseconds) { cut.showUnreadMarker.value shouldBe true }
    }

    @Test
    fun `showUnreadMarker » show the unread marker when subsequent event is added`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        val timeline = timeline(roomServiceMock, roomId) { +messageEvent(sender = aliceId) { text("Hi!") } }
        val cut = cut(eventId = EventId("0"))
        timeline.fullyReadEventIndex.value = 0
        backgroundScope.launch { cut.showUnreadMarker.collect() }
        eventually(100.milliseconds) { cut.showUnreadMarker.value shouldBe false }
        timeline.addEvents { +messageEvent(sender = bobId) { text("Hi!") } }
        eventually(100.milliseconds) { cut.showUnreadMarker.value shouldBe true }
    }
    ...
    fun `showUnreadMarker » not show the unread marker when subsequent event is added but by us`() = runTest {
        ...
        backgroundScope.launch { cut.showUnreadMarker.collect() }
        eventually(100.milliseconds) { cut.showUnreadMarker.value shouldBe false }
        timeline.addEvents { +messageEvent(sender = usId) { text("Hi!") } }
        eventually(100.milliseconds) { cut.showUnreadMarker.value shouldBe false }
    }
    ...
    fun `element » not create a new viewModel when a new message is sent afterwards`() = runTest {
        every { roomServiceMock.getOutbox(roomId) } returns flowOf(listOf())
        val timeline =
            timeline(roomServiceMock, roomId) { +messageEvent(sender = aliceId) { text("Don't change my viewModel!") } }
        val event = timeline.eventsInStore.value[0]
        ...
        timeline.addEvents {
            +messageEvent(sender = bobId) { text("This shouldn't change the former messages viewModel") }
        }
    ...
    fun `isReplaced » should be true when new content in outbox`() = runTest {
        ...
        backgroundScope.launch { cut.isReplaced.collect() }
        eventually(100.milliseconds) { cut.isReplaced.value shouldBe true }
    }
    ...
    fun `isReplaced » should be true when replaced`() = runTest {
        ...
        timeline(roomServiceMock, roomId) {
            +timelineEvent
            +messageEvent(sender = aliceId) { text("edit") }
        }
        val cut = cut(eventId = eventId)
        backgroundScope.launch { cut.isReplaced.collect() }
        eventually(100.milliseconds) { cut.isReplaced.value shouldBe true }
    ...
    fun `errorIfReplaced » should show an error when sendError of replacing event is not null`() = runTest {
        val sendError = RoomOutboxMessage.SendError.Unknown(ErrorResponse.Unknown("Too large"))
        every { roomServiceMock.getOutbox(roomId) } returns
            flowOf(
                listOf(
                    flowOf(
        ...
        eventually(100.milliseconds) { cut.sendError.value shouldNotBe null }
    }
    ...
    fun `redact » should redact via warning if enabled`() = runTest {
        ...
        delay(100.milliseconds)
        cut.redact()
        eventually(100.milliseconds) { cut.showRedactionWarning.value shouldBe true }
        continually(100.milliseconds) { redactCalled shouldBe false }
    ...
    private fun TestScope.cut(
        ...
        return TimelineElementHolderViewModelImpl(
            viewModelContext = testMatrixClientViewModelContext(di = di, userId = usId),
            key = "$roomId-$eventId",
            roomId = roomId,
            eventId = eventId,
            ...
            formattedTime = "07:24",
            showUnreadMarker =
                roomServiceMock.getAccountData(roomId, FullyReadEventContent::class).map { it?.eventId == eventId },
