...
class TypingInfoTest {
    ...
    fun `return null when no user is typing anything`() = runTest {
        typingInfo(matrixClientMock, roomId, i18n, TypingEventContent(users = setOf())) shouldBe null
    }
    ...
    fun `return writing message when is direct room`() = runTest {
        ...
        typingInfo(matrixClientMock, roomId, i18n, TypingEventContent(users = setOf(user2))) shouldBe "is typing..."
    }
    ...
    fun `return username is typing when single user is writing in a group`() = runTest {
        ...
        typingInfo(matrixClientMock, roomId, i18n, TypingEventContent(users = setOf(user2))) shouldBe "2 is typing..."
    }

    @Test
    fun `return all names when multiple users are writing in a group`() = runTest {
        every { roomServiceMock.getById(roomId) } returns MutableStateFlow(Room(roomId, isDirect = false))
        typingInfo(matrixClientMock, roomId, i18n, TypingEventContent(users = setOf(user2, user3, user4))) shouldBe
            "2, 3 and 4 are typing..."
    }

    @Test
    fun `filter current user from the list`() = runTest {
        every { roomServiceMock.getById(roomId) } returns MutableStateFlow(Room(roomId, isDirect = false))
        typingInfo(matrixClientMock, roomId, i18n, TypingEventContent(users = setOf(user1, user2))) shouldBe
            "2 is typing..."
    }
    ...
    fun `cut more than 4 users writing in parallel and return a shorter message`() = runTest {
        ...
        typingInfo(
            ...
            roomId,
            i18n,
            TypingEventContent(users = setOf(user2, user3, user4, user5, user6)),
    ...
    private fun mockUser(userId: UserId, name: String) {
        every { userServiceMock.getById(roomId, userId) } returns
            MutableStateFlow(RoomUser(roomId, userId, name, memberEvent()))
    }
