...
class MatrixClientInitializationViewModelTest {
    ...
    init {
        ...
        everySuspend { matrixClientsMock.initFromStoreResult } returns
            MutableStateFlow(MatrixClients.InitFromStoreResult(success = emptySet(), failures = emptyMap()))
    }
    ...
    fun `call onNoAccounts when no accounts are present`() = runTest {
        matrixClientInitializationViewModel(accounts = emptyMap(), selectedAccount = null)
        eventually(4.seconds) { verify { onNoAccountsMock.invoke() } }
    }
    ...
    fun `leave the currently active account if the account can be found`() = runTest {
        val settings =
            matrixClientInitializationViewModel(
                accounts =
                    mapOf(
                        UserId("user1", "local.local") to
                            MatrixMessengerAccountSettingsBase.withConfigDefaults(
                                config = MatrixMessengerConfiguration(),
                                displayColor = null,
                            )
                    ),
                selectedAccount = UserId("user1", "local.local"),
            )
        continually(2.seconds) { settings.value.base.accounts.size shouldBe 1 }
    }
    ...
    fun `select the only left account when the currently active account is not present anymore`() = runTest {
        ...
        eventually(2.seconds) { settings.value.base.selectedAccount shouldBe UserId("user1", "local.local") }
    }
    ...
    fun `select all accounts if the currently active account is not present anymore`() = runTest {
        ...
        eventually(2.seconds) { settings.value.base.selectedAccount shouldBe null }
    }
            ...
            koinApplication {
                    modules(
                        createTestDefaultTrixnityMessengerModules() +
                            module { single<MatrixClients> { matrixClientsMock } }
                    )
                }
                .koin
        val settings = di.get<MatrixMessengerSettingsHolder>()
        accounts.forEach { (account, accountSettings) -> settings.create(account, accountSettings) }
