...
class BlockedContactsSettingsViewModelTest {
    ...
    init {
        blockedContactsForUser1.value = listOf(contact3, contact1)
    ...
    fun `return a list and count of blocked contacts per account`() = runTest {
        ...
        eventually(1.seconds) {
            viewModel1.blockedContactsList.value shouldBe
                listOf(BlockedContact(contact3, false), BlockedContact(contact1, false))
        ...
        eventually(1.seconds) {
            viewModel2.blockedContactsList.value shouldBe
                listOf(BlockedContact(contact1, false), BlockedContact(contact2, false))
    ...
    fun `mark a blocked contact as unblocking and then unblock them`() = runTest {
        ...
        everySuspend { userBlockingMock.unblockUser(matrixClientMock1, contact3, any(), any()) } calls
            {
                isRequestCompleted.first { it }
            }
        ...
        eventually(1.seconds) {
            viewModel.blockedContactsList.value shouldBe
                listOf(BlockedContact(contact3, false), BlockedContact(contact1, false))
        }
        ...
        eventually(1.seconds) {
            viewModel.blockedContactsList.value shouldBe
                listOf(BlockedContact(contact3, true), BlockedContact(contact1, false))
        }

        blockedContactsForUser1.value = listOf(contact1)
        isRequestCompleted.value = true
        eventually(1.seconds) { viewModel.blockedContactsList.value shouldBe listOf(BlockedContact(contact1, false)) }
    }

    private fun TestScope.blockedContactsSettingsViewModel(user: User): BlockedContactsSettingsViewModel {
        val di =
            koinApplication {
                    modules(
                        createTestDefaultTrixnityMessengerModules(
                            mapOf(User.USER1.userId to matrixClientMock1, User.USER2.userId to matrixClientMock2)
                        ) + module { single { userBlockingMock } }
                    )
                }
                .koin
        return BlockedContactsSettingsViewModelImpl(
            viewModelContext = testMatrixClientViewModelContext(userId = user.userId, di = di),
