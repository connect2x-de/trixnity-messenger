...
class MatrixMessengerService :
    SingletonService<MatrixMessenger>({ applicationContext ->
...
fun isMatrixMessengerServiceEnabled(context: Context) =
    try {
        context.packageManager.getServiceInfo(ComponentName(context, MatrixMessengerService::class.java), 0).enabled
...
suspend fun <T> withDiFromService(context: Context, block: suspend (di: Koin) -> T): T =
    when {
        isMatrixMessengerServiceEnabled(context) -> {
            withSingletonService<T, MatrixMessenger, MatrixMessengerService>(context) { block(it.di) }
        }

        isMatrixMultiMessengerServiceEnabled(context) ->
            withSingletonService<T, MatrixMultiMessenger, MatrixMultiMessengerService>(context) { block(it.di) }
