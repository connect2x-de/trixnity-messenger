...
object MatrixMultiMessengerService : SingletonService<MatrixMultiMessenger>() {
    override suspend fun factory(): MatrixMultiMessenger = MatrixMultiMessenger.create(configuration = configuration)
...
suspend fun <T> withMatrixMultiMessengerFromService(
    ...
    val matrixMultiMessenger =
        MatrixMultiMessengerService.get() ?: throw IllegalStateException("MatrixMultiMessengerService is not enabled")
    return block(matrixMultiMessenger)
}

val activeDI: Koin
    get() =
        checkNotNull(MatrixMultiMessengerService.get()?.di ?: MatrixMessengerService.get()?.di) {
