...
class ApplicationDelegate : UIApplicationDelegateProtocol, NSObject {
    ...
    override fun application(application: UIApplication, willFinishLaunchingWithOptions: Map<Any?, *>?): Boolean =
        delegate
            .willFinishLaunching(application = application, launchOptions = willFinishLaunchingWithOptions.unsafeCast())
            .orTrue

    @ObjCSignatureOverride
    override fun application(application: UIApplication, didFinishLaunchingWithOptions: Map<Any?, *>?): Boolean =
        delegate
            .didFinishLaunching(application = application, launchOptions = didFinishLaunchingWithOptions.unsafeCast())
            .orTrue
    ...
    override fun application(application: UIApplication, didDiscardSceneSessions: Set<*>) =
    ...
    override fun applicationWillTerminate(application: UIApplication) =
        delegate.willTerminate(application = application)

    override fun applicationProtectedDataDidBecomeAvailable(application: UIApplication) =
        delegate.protectedDataDidBecomeAvailable(application = application)

    override fun applicationProtectedDataWillBecomeUnavailable(application: UIApplication) =
        delegate.protectedDataWillBecomeUnavailable(application = application)

    override fun applicationDidReceiveMemoryWarning(application: UIApplication) =
        delegate.didReceiveMemoryWarning(application = application)

    override fun applicationSignificantTimeChange(application: UIApplication) =
        delegate.significantTimeChange(application = application)

    @ObjCSignatureOverride
    override fun application(application: UIApplication, shouldSaveSecureApplicationState: NSCoder): Boolean =
        delegate
            .shouldSaveSecureApplicationState(application = application, coder = shouldSaveSecureApplicationState)
            .orFalse

    @ObjCSignatureOverride
    override fun application(application: UIApplication, shouldRestoreSecureApplicationState: NSCoder): Boolean =
        delegate
            .shouldRestoreSecureApplicationState(application = application, coder = shouldRestoreSecureApplicationState)
            .orTrue
    ...
    @ObjCSignatureOverride
    override fun application(application: UIApplication, willEncodeRestorableStateWithCoder: NSCoder) =
        delegate.willEncodeRestorableState(application = application, coder = willEncodeRestorableStateWithCoder)

    @ObjCSignatureOverride
    override fun application(application: UIApplication, didDecodeRestorableStateWithCoder: NSCoder) =
        delegate.didDecodeRestorableState(application = application, coder = didDecodeRestorableStateWithCoder)
    ...
    override fun application(application: UIApplication, didFailToRegisterForRemoteNotificationsWithError: NSError) =
    ...
    override fun application(
        ...
        scope.delegate(fetchCompletionHandler, { UIBackgroundFetchResultNoData }) {
            delegate.didReceiveRemoteNotification(application = application, userInfo = didReceiveRemoteNotification)
        }
    ...
    override fun applicationShouldRequestHealthAuthorization(application: UIApplication) =
        delegate.shouldRequestHealthAuthorization(application = application)
    ...
    override fun application(application: UIApplication, handlerForIntent: INIntent): Any? =
        delegate.handlerForIntent(application = application, intent = handlerForIntent).orNull

    override fun applicationShouldAutomaticallyLocalizeKeyCommands(application: UIApplication): Boolean =
        delegate.shouldAutomaticallyLocalizeKeyCommands(application = application).orTrue
    ...
    override fun application(
        ...
        delegate
            .supportedInterfaceOrientations(application = application, window = supportedInterfaceOrientationsForWindow)
