...
class SecretByteArraysImpl(
    ...
    private suspend fun rotateKeys(
        ...
        val newKey =
            secretByteArrayKeyProviders
                .fold((null to null) as Pair<String?, SecretByteArrayKeyProvider.RotateResult?>) {
                    (inputProviderId, inputRotateResult),
                ...
                .second
                ?.let { it.getNewKey?.invoke(keySize) }

        val newSecretByteArrays =
            secretByteArraySettings.secrets.orEmpty().mapValues { (id, secretByteArray) ->
                val byteArray = get(id, secretByteArray, oldKey)
                get(id, byteArray, newKey)
            }
        val newSettings =
            SecretByteArraySettings(secrets = newSecretByteArrays, keyInfo = newSecretByteArrayKeyInfos, key = newKey)
        settings.update<SecretByteArraySettings> { newSettings }
        newSettings
    }
    ...
    private suspend fun getKey(size: Int): ByteArray? {
        ...
        val orderedSecretByteArrayKeyProviders = getOrderedSecretByteArrayKeyProviders(secretByteArraySettings)
        log.debug {
            "getKey (size=$size, orderedSecretByteArrayKeyProviders=${orderedSecretByteArrayKeyProviders.map { it.id }})"
        }
        val key =
            orderedSecretByteArrayKeyProviders
                .fold(null as GetKey?) { getInputKey, secretByteArrayKeyProvider ->
    ...
    private fun getOrderedSecretByteArrayKeyProviders(
        secretByteArraySettings: SecretByteArraySettings?
    ): List<SecretByteArrayKeyProvider> {
        val providerInfos = secretByteArraySettings?.keyInfo.orEmpty()

        val rootProviderId = providerInfos.entries.find { it.value.dependsOn == null }?.key ?: return emptyList()
