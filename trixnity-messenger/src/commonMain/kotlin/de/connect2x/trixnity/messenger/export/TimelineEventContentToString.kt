            ...
            contentResult != null ->
                when {
                        ...
                        when (val content = contentResult.getOrThrow()) {
                            ...
                            is StateEventContent ->
                                when (content) {
                                    ...
                                    is CanonicalAliasEventContent ->
                                        i18n.exportRoomCanonicalAlias(
                                            listOfNotNull(content.alias) + content.aliases.orEmpty()
                                        )
                                    ...
                                    is TopicEventContent ->
                                        i18n.exportRoomTopic(content.topic?.text?.plain ?: content.legacy.topic)

                                    is EncryptionEventContent -> i18n.exportRoomEncryption()
                                    is HistoryVisibilityEventContent ->
                                        i18n.exportRoomHistoryVisibility(content.historyVisibility.name)
                                    is GuestAccessEventContent -> i18n.exportRoomGuestAccess(content.guestAccess.name)
                                    is TombstoneEventContent ->
                                        i18n.exportRoomTombstone(content.body, content.replacementRoom)
                            ...
                            is RedactedEventContent ->
                                i18n.exportRoomRedacted(
                                    ...
                                    (timelineEvent.event.unsigned?.redactedBecause?.content as? RedactionEventContent)
                                        ?.reason,
                                )
                    ...
                    contentResult.isFailure -> {
                        val error =
                            contentResult.exceptionOrNull() ?: throw IllegalStateException("result in unexpected state")
                        if (error is TimelineEvent.TimelineEventContentError) {
                            when (error) {
                                TimelineEvent.TimelineEventContentError.DecryptionAlgorithmNotSupported ->
                                    i18n.exportRoomDecryptionError()
