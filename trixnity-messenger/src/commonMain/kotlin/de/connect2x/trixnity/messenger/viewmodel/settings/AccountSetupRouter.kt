...
class AccountSetupRouter(
    ...
    val stack =
        viewModelContext
            .childContext("accountSetupViewModel")
            .childStack(
                source = navigation,
        ...
        when (config) {
            ...
            is Config.ShowAccountSetup -> {
                ...
                Wrapper.ShowAccountSetup(
                    get<AccountSetupViewModelFactory>()
                        .create(
                            viewModelContext.childContext("ShowAccountSetup", componentContext, config.userId),
                            ::onSetupClose,
                            onStartVerification,
                        )
                )
    ...
    fun onSetupClose(userId: UserId) {
        ...
        coroutineScope.launch {
            settings.update<MatrixMessengerAccountSettingsBase>(userId) { it.copy(accountSetupFinished = true) }
    ...
    fun onCloseSelfVerification(userId: UserId, completedVerification: Boolean) {
        ...
        if (
            activeInstance.configuration is Config.ShowAccountSetup &&
                (activeInstance.configuration as Config.ShowAccountSetup).userId == userId &&
                activeInstance.instance is Wrapper.ShowAccountSetup
        ) {
            (activeInstance.instance as Wrapper.ShowAccountSetup)
                .viewModel
                .changeVerificationCompleteStatus(completedVerification)
        }
    }
    ...
    @Serializable
    sealed class Config {
        @Serializable data class ShowAccountSetup(val userId: UserId) : Config()
