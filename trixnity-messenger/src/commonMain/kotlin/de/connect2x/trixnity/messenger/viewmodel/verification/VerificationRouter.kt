...
class VerificationRouter(
    ...
    private fun createChild(config: Config, componentContext: ComponentContext): Wrapper =
        when (config) {
            is Config.DeviceVerification ->
                Wrapper.Verification(
                    viewModelContext
                        .get<VerificationViewModelFactory>()
                        .create(
                            viewModelContext =
                                viewModelContext.childContext("DeviceVerification", componentContext, config.userId),
            ...
            is Config.UserVerification ->
                Wrapper.Verification(
                    viewModelContext
                        .get<VerificationViewModelFactory>()
                        .create(
                            viewModelContext =
                                viewModelContext.childContext("UserVerification", componentContext, config.userId),
                            onCloseVerification = ::closeVerification,
                            onRedoSelfVerification = { onRedoSelfVerification(config.userId) },
    ...
    @Serializable
    sealed class Config {
        @Serializable data class DeviceVerification(val userId: UserId) : Config()
