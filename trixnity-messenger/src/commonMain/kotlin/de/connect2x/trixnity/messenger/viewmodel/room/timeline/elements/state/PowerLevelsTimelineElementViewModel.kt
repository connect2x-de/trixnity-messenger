...
interface PowerLevelsTimelineElementViewModelFactory : TimelineElementViewModelFactory<PowerLevelsEventContent> {
    ...
    override val supports: KClass<PowerLevelsEventContent>
        get() = PowerLevelsEventContent::class
...
private fun <T, V : Any> findMapDifference(oldMap: Map<T, V>, newMap: Map<T, V>): MapDifference<T, V> {
    ...
    val removedEntries = allKeys.mapValueNotNull { (old, new) -> if (new == null) old else null }
    val newEntries = allKeys.mapValueNotNull { (old, new) -> if (old == null) new else null }

    val changedEntries = allKeys.mapValueNotNull { (old, new) ->
        if (old != null && new != null && old != new) old to new else null
    }
