...
interface BackHandler {
     ...
     *
     * @param callback The callback to be added with higher priority values taking precedence over lower ones in the
     *   stack evaluation.
     */
...
class BackHandlerImpl : BackHandler {
    ...
    override fun registerBackCallback(callback: BackCallback) {
        val indexToAdd =
            _backCallbackStack
                .indexOfFirst { listElement -> callback.priority >= listElement.priority }
                .coerceAtLeast(0)
    ...
    override fun Lifecycle.registerBackCallbackWithLifecycle(callback: BackCallback) {
        ...
        this.let { it.doOnDestroy { unregisterCallback(callback) } }
    }
