...
class SelfVerificationRouter(
        ...
        when (selfVerificationConfig) {
            ...
            is Config.CrossSigningBootstrap -> {
                Wrapper.CrossSigningBootstrap(
                    viewModelContext
                        .get<CrossSigningBootstrapViewModelFactory>()
                        .create(
                            viewModelContext =
                                viewModelContext.childContext(
                    ...
                    launch {
                        ...
                        client.verification.getSelfVerificationMethods().distinctUntilChanged().collect {
                            if (it is VerificationService.SelfVerificationMethods.NoCrossSigningEnabled) {
                                log.debug {
                                    "found Cross Signing to be disabled for account $userId, starting bootstrap"
                                }
    ...
    private fun closeCrossSigningBootstrap(userId: UserId) =
        viewModelContext.coroutineScope.launch {
        ...
        viewModelContext.coroutineScope.launch {
            ...
            selfVerifications.collect { currentSelfVerifications ->
                ...
                if (nextAccountToVerify != null) {
                    navigation.replaceCurrentSuspending(Config.SelfVerification(nextAccountToVerify))
                } else {
        ...
        coroutineScope.launch {
            crossSigningBootstraps.collect { currentBootstraps ->
                ...
                if (nextBootstrap != null) {
                    navigation.replaceCurrentSuspending(Config.CrossSigningBootstrap(nextBootstrap))
                }
            }
        }
    }
    ...
    sealed class Config {
        ...
        @Serializable data class SelfVerification(val userId: UserId) : Config()

        @Serializable data class RedoSelfVerification(val userId: UserId) : Config()

        @Serializable data class CrossSigningBootstrap(val userId: UserId) : Config()
    }
}
