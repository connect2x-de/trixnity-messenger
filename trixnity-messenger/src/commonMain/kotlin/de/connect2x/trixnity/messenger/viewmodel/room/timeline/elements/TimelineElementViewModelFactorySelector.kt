...
class TimelineElementViewModelFactorySelectorImpl(
    ...
    companion object {
        private val log: Logger =
            Logger(
                "de.connect2x.trixnity.messenger.viewmodel.room.timeline.elements.TimelineElementViewModelFactorySelectorImpl"
            )
    }
    ...
    private sealed interface Mapping {
        ...
        data class Exist(val factory: TimelineElementViewModelFactory<RoomEventContent>) : Mapping
    ...
    override suspend fun create(
        ...
        when {
            ...
            content == null ->
                encryptedWaitTimelineElementViewModelFactory.create(viewModelContext = viewModelContext)
                ...
                content.fold(
                    ...
                    onSuccess = { decryptedContent ->
                        ...
                        // This has to run on the Main Thread as ViewModels can create routers or stacks which have to
                        // be
                        // created on the Main Thread to prevent
                        // com.arkivanov.decompose.mainthread.NotOnMainThreadException
    ...
    private suspend fun findFactory(
        ...
        if (ignoreReplacedEvents && isReplaceEvent(content))
            return null.also { log.debug { "Ignoring replace event: $content" } }

        val contentClass = content::class
        return (factoryMapping.read { get(contentClass) }
                ?: run {
                    val foundFactory =
                        factories.firstOrNull { it.supports.isInstance(content) }
                            ?: run {
                                log.warn {
