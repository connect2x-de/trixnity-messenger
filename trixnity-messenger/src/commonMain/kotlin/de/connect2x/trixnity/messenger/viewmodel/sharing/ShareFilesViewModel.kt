...
interface ShareDataViewModelFactory {
    fun create(viewModelContext: ViewModelContext, sharedData: SharedData, onClose: () -> Unit): ShareDataViewModel {
        return SharedDataViewModelImpl(viewModelContext, sharedData, onClose)
    }
...
class SharedDataViewModelImpl(
    ...
    override val roomList: RoomListViewModel =
        roomListViewModelFactory.create(
    ...
    private suspend fun sendPlainText(body: String, matrixClient: MatrixClient, roomId: RoomId) {
        // TODO: unified SendMessage viewmodel
        matrixClient.room.sendMessage(roomId) { text(body) }
    }
