...
suspend fun typingInfo(
    ...
    return when (usersTyping.size) {
        0 -> null
        1 -> {
            val username = usersTyping[0].let { matrixClient.user.getById(roomId, it).first()?.name ?: it.full }
            val isDirect = matrixClient.room.getById(roomId).first()?.isDirect ?: false
            when {
        ...
        in 2..4 -> {
            val usernames = usersTyping.map { matrixClient.user.getById(roomId, it).first()?.name ?: it.full }

            i18n.roomHeaderTypingMultiple(
                i18n.commonAnd(usernames.take(usernames.size - 1).joinToString(), usernames.last())
            )
        }
