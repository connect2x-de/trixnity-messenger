...
fun <T> Flow<T>.takeWhileInclusive(pred: suspend (T) -> Boolean): Flow<T> = transformWhile { value ->
    if (pred(value)) {
        emit(value)
        true
    } else {
...
fun <T> Flow<T>.throttleFirst(delay: Long): Flow<T> = transform {
    emit(it)
    delay(delay)
}

fun <T> Flow<T>.throttleFirst(delay: Duration): Flow<T> = throttleFirst(delay.inWholeMilliseconds)

fun <T> Flow<T>.debounceAfterFirst(delay: Duration): Flow<T> = channelFlow {
