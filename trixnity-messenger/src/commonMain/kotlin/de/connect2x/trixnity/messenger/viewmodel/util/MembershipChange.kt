...
internal enum class MembershipChange {
    ...
    companion object {
        ...
        // * -> LEAVE => Leave
        fun of(from: Membership?, to: Membership, self: Boolean): MembershipChange =
            when (to) {
                Membership.INVITE -> INVITE
                Membership.JOIN -> JOIN
                Membership.LEAVE -> when (from) {
                    Membership.BAN -> UNBAN
                    Membership.INVITE -> if (self) INVITE_REJECT else INVITE_REVOKE
                    else -> if (self) LEAVE else KICK
                }
