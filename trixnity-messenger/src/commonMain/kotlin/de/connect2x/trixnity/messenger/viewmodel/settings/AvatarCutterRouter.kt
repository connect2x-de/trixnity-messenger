...
class AvatarCutterRouter(private val viewModelContext: ViewModelContext) {
    ...
    private fun createChild(config: Config, componentContext: ComponentContext): Wrapper =
        when (config) {
            ...
            is Config.AvatarCutterUserProfile ->
                Wrapper.AvatarCutter(
                    viewModelContext
                        .get<AvatarCutterViewModelFactory>()
                        .create(
                            viewModelContext =
                                viewModelContext.childContext(
                                    "AvatarCutterUserProfile",
                                    componentContext,
                                    config.userId,
                                ),
                            file = config.file,
            ...
            is Config.AvatarCutterRoom ->
                Wrapper.AvatarCutter(
                    viewModelContext
                        .get<AvatarCutterViewModelFactory>()
                        .create(
                            viewModelContext =
                                viewModelContext.childContext("AvatarCutterRoom", componentContext, config.userId),
    ...
    sealed class Config {
        data class AvatarCutterUserProfile(val userId: UserId, val file: FileDescriptor) : Config()

        data class AvatarCutterRoom(val userId: UserId, val roomId: RoomId, val file: FileDescriptor) : Config()
