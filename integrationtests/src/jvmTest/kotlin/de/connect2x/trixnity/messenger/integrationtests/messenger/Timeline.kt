...
suspend fun MatrixMessengerWithRoot.sendMessage(roomId: RoomId, message: String) =
    with(root) {
...
suspend fun MatrixMessengerWithRoot.findMessage(roomId: RoomId, message: String): Boolean =
    with(root) {
...
suspend fun MatrixMessengerWithRoot.initiateUserVerification(roomId: RoomId, userId: UserId) =
    with(root) {
...
suspend fun MatrixMessengerWithRoot.acceptUserVerification(roomId: RoomId, otherUserId: UserId) =
    with(root) {
...
suspend fun MatrixMessengerWithRoot.startVerificationWithEmoji(roomId: RoomId) =
    with(root) {
        log.info { "  +- start verification with emoji (client1)" }
        withTimeout(20.seconds) {
            val verificationViewModel = verificationViewModel(roomId)
            val selectVerification =
                verificationViewModel.stack
                    .waitFor(VerificationViewModel.Wrapper.SelectVerificationMethod::class)
                    .viewModel
...
suspend fun MatrixMessengerWithRoot.acceptVerificationWithEmoji(roomId: RoomId) =
    with(root) {
        ...
        withTimeout(20.seconds) {
            ...
            val compareViewModel =
                verificationViewModel.stack
                    .waitFor(VerificationViewModel.Wrapper.CompareEmojisOrNumbers::class)
                    .viewModel
...
suspend fun MatrixMessengerWithRoot.originalClientAcceptVerificationWithEmoji(roomId: RoomId) =
    with(root) {
...
private suspend inline fun <
    reified T : TimelineElementViewModel<*>,
    reified H : BaseTimelineElementHolderViewModel,
> findTimelineElement(roomViewModel: RoomViewModel): T {
    val timelineViewModel = roomViewModel.timelineStack.waitFor(TimelineRouter.Wrapper.View::class).viewModel
    timelineViewModel.elements.first { it.isNotEmpty() }
    timelineViewModel.elements
        .flatMapLatest { vms ->
            initViewState(timelineViewModel, vms)
            combine(vms.filterIsInstance<H>().map { it.element.filterNotNull() }) { it.toList() }
        }
        .first { vms ->
            log.debug { "+++ vms: $vms" }
            vms.any { vm ->
    ...
    return timelineViewModel.elements
        .map { it.filterIsInstance<H>().find { vm -> vm.element.filterNotNull().first() is T } }
        .filterNotNull()
        .first()
        .element
        .filterNotNull()
