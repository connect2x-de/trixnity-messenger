...
suspend fun MatrixMessengerWithRoot.login(
    ...
    with(root) {
        ...
        mainViewModel.accountSetupRouterStack
            .waitFor(AccountSetupRouter.Wrapper.ShowAccountSetup::class)
            .viewModel
            .closeAccountSetup()
        mainViewModel.accountSetupRouterStack.waitFor(AccountSetupRouter.Wrapper.None::class)
        val verification =
            withTimeoutOrNull(15.seconds) {
                mainViewModel.selfVerificationStack
                    .toFlow()
                    .first { childStack ->
        ...
        if (verification == null) {
            ...
            val verificationView =
                mainViewModel.selfVerificationStack.waitFor(SelfVerificationRouter.Wrapper.View::class)
            if (recoveryKey != null) {
...
private suspend fun RootViewModel.addMatrixAccountViaPassword(serverUrl: String, username: String, password: String) {
    ...
    val passwordLoginViewModel = stack.waitFor(RootRouter.Wrapper.PasswordLogin::class).viewModel
    addMatrixAccountViewModel.serverUrl.update(serverUrl)
...
private suspend fun selfVerify(verification: SelfVerificationRouter.Wrapper, recoveryKey: String) {
    log.info { "  +- self verification with recovery key" }
    val selfVerificationViewModel = (verification as SelfVerificationRouter.Wrapper.View).viewModel
    selfVerificationViewModel.waitForAvailableVerificationMethods()
    selfVerificationViewModel.selfVerificationMethods.first {
        it.any { selfVerificationMethod -> selfVerificationMethod is SelfVerificationMethod.AesHmacSha2RecoveryKey }
    }
...
private suspend fun selfVerify(
    ...
    selfVerificationViewModel.waitForAvailableVerificationMethods()
    selfVerificationViewModel.selfVerificationMethods.first {
        it.any { selfVerificationMethod ->
            selfVerificationMethod is SelfVerificationMethod.CrossSignedDeviceVerification
        }
    }
    selfVerificationViewModel.launchVerification(
        selfVerificationViewModel.selfVerificationMethods.value.find {
    ...
    val otherVerificationViewModel =
        otherMainViewModel.deviceVerificationRouterStack
            .waitFor(VerificationRouter.Wrapper.Verification::class)
            .viewModel
    ...
    selectVerificationMethodViewModel.acceptVerificationMethod(
        selectVerificationMethodViewModel.verificationMethods[0].first
    )
    ...
    mainViewModel.deviceVerificationRouterStack.toFlow().first {
        it.active.configuration is VerificationRouter.Config.None
    }
    otherMainViewModel.deviceVerificationRouterStack.toFlow().first {
        it.active.configuration is VerificationRouter.Config.None
    }
}
