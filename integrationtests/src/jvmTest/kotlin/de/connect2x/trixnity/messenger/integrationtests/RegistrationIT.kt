...
class RegistrationIT {
    ...
    @Container val synapseDocker = synapseDocker(useRegistrationToken = true)
    ...
    fun shouldRegisterNewUserWithRegistrationToken(): Unit = runBlockingWithTimeout {
        ...
        val body =
            httpClient
                .post("$baseUrl/_synapse/admin/v1/registration_tokens/new?access_token=$accessToken") {
        ...
        "\"token\":\\s*\"([^\"]*)\"".toRegex().find(body)?.groupValues?.getOrNull(1)?.let { token ->
            ...
            messenger.registerAccountWithToken(serverUrl = baseUrl, registrationToken = token)
    ...
    private suspend fun createAdminAccount(httpClient: HttpClient, baseUrl: String): String {
        try {
            ...
            "\"nonce\":\\s*\"([^\"]*)\"".toRegex().find(nonceBody)?.groupValues?.getOrNull(1)?.let { nonce ->
                ...
                macAlgorithm.init(
                    SecretKeySpec("8XRC-cObB+9MaK+f~n=,9TL&;5+#w6Djp&cI3:FFJay=h4FkHS".encodeToByteArray(), "HmacSHA1")
                )
                val mac =
                    macAlgorithm
                        .doFinal(
                ...
                val registrationBody =
                    httpClient
                        .post("$baseUrl/_synapse/admin/v1/register") {
                            contentType(ContentType.Application.Json)
                            setBody(
                                """
                            {
                        ...
                        .bodyAsText()
                "\"access_token\":\\s*\"([^\"]*)\"".toRegex().find(registrationBody)?.groupValues?.getOrNull(1)?.let {
                    accessToken ->
                    return accessToken
