...
class DragAndDrop(private val dragAndDropHandler: DragAndDropHandlerBase, private val fileSystem: FileSystem) :
    DropTargetListener {
    companion object {
        private val log: Logger = Logger("de.connect2x.trixnity.messenger.compose.view.DragAndDrop")
    }
    ...
    override fun dragEnter(dtde: DropTargetDragEvent?) {
        ...
        dtde?.transferable?.getTransferData(DataFlavor.javaFileListFlavor)?.let { transferData ->
            ...
            dragAndDropHandler.drag(
                files.mapNotNull {
                    if (it is File) PathFileDescriptor(path = it.absolutePath.toPath(), fileSystem = fileSystem)
                    else null
                }
            )
        }
    ...
    override fun drop(dtde: DropTargetDropEvent?) {
        ...
        dtde?.transferable?.getTransferData(DataFlavor.javaFileListFlavor)?.let { transferData ->
            ...
            dragAndDropHandler.drop(
                files.mapNotNull { if (it is File) PathFileDescriptor(it.absolutePath.toPath(), fileSystem) else null }
            )
