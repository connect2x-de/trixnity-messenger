...
actual fun PDFReader(
    ...
    val dpPixel = remember { mutableStateOf<Int?>(null) }
    ...
    LaunchedEffect(documentWidth.value, viewSize.width) {
        documentWidth.value?.toFloat()?.let {
            ...
            val dpi = (viewSize.width / it * scale / density * 2f).coerceAtMost(maxDpi)
            dpPixel.value = (dpi / it).toInt()
        }
    }
    ...
    val readerError = remember { mutableStateOf<String?>(null) }
    ...
    Box(
        ...
        if (readerError.value == null) {
            renderer.value?.let { pdfReader ->
                    ...
                    LazyColumn(
                        ...
                        content = {
                            items(count = pdfReader.pageCount, key = { it }) { pageId ->
                                val cacheKey = "$pageId:${dpPixel.value?.toInt()}"
