...
import de.connect2x.trixnity.messenger.compose.view.verification.SelfVerificationWizard
...
private fun SelfVerificationPreview() {
    InitMessengerPreview { SelfVerificationWizard(SelfVerificationViewModelPreview()) }
}
...
private fun SelfVerificationPreviewWithRecoveryKey() {
    InitMessengerPreview {
        SelfVerificationWizard(SelfVerificationViewModelPreview(aesHmacSha2RecoveryKey = aesHmacSha2RecoveryKey()))
    }
}
...
private fun SelfVerificationPreviewOnResetWarning() {
    ...
    LaunchedEffect(model) { model.resetRecoveryWarning() }

    InitMessengerPreview { SelfVerificationWizard(model) }
}

@Preview(showBackground = true, backgroundColor = 0xFFFFFF)
@Composable
private fun RedoSelfVerificationPreview() {
    InitMessengerPreview { RedoSelfVerificationWizard(RedoSelfVerificationViewModelPreview()) }
}

private val demoUserId = UserId("@demo:timmy-messenger.de")

private fun aesHmacSha2RecoveryKey() =
    AesHmacSha2RecoveryKey(
        keyTrustService =
            object : KeyTrustService {
...
private class SelfVerificationViewModelPreview(
    ...
    override val selfVerificationMethods: MutableStateFlow<Set<SelfVerificationMethod>> =
        MutableStateFlow(
            setOf(aesHmacSha2RecoveryKey, aesHmacSha2RecoveryKeyWithPbkdf2Passphrase, crossSignedDeviceVerification)
                .filterNotNull()
                .toSet()
        )
