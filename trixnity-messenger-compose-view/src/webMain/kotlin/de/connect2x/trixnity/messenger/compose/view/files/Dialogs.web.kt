...
/**
 * This component invokes a file picker by which the user can select a file to be uploaded for where it may be needed or
 * desired.
 *
 * @param onFileSelect is invoked on successful selection of a file which then provides a `FileDescriptor` with the byte
 *   stream.
 * @param onCloseLoadFileDialog is invoked on completion or conclusion of the file picker.
 * @param availableTypes indicates to the file picker which types of files or media are being considered for file
 *   selection.
 */
@Composable
...
/**
 * This component triggers a file download stream directly. For this it requires a @param fileName, @param mimeType and
 * the byte stream via @param downloadFile. If an error is provided via @param error, it will display an alert instead.
 *
