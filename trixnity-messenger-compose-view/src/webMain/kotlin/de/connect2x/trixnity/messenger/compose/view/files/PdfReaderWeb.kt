...
suspend fun PdfReaderWeb(blob: Blob): PdfReaderWeb {
    ...
    val documentWidth =
        doc.getPage(1.toJsNumber()).await().getViewport(GetViewportParameters(1.toJsNumber())).width.toInt()
...
internal constructor(
    ...
    suspend fun renderPage(pageIndex: Int, dpi: Float): ImageBitmap? {
        if (pageIndex > pageSize)
            throw IllegalArgumentException("Page index must be smaller or equal to Page size ($pageSize)")
    ...
    private fun getOrCreatePageCanvas(pageId: Int): HTMLCanvasElement {
        return (document.getElementById(ElementId("pdf-canvas-page-$pageId"))
            ?: document.createElement("canvas").apply { id = ElementId("pdf-canvas-page-$pageId") })
