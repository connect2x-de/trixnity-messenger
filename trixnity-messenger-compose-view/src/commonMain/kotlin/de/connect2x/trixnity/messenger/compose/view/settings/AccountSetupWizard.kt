...
/**
 * Represents a step in the Account Setup Wizard, utilizing a unique identifier for each step. This class serves as a
 * base for defining various specific steps within the wizard process. Inherit from this class to create or replace
 * steps.
 *
...
/**
 * Represents a list of steps in the account setup wizard process. To change certain or add new steps, replace
 * `AccountSetupWizardStepListImpl`
 */
...
/**
 * This interface allows for the creation of custom steps within the wizard by providing its own specific logic and user
 * interface components as part of the wizard flow.
 *
 * This is built upon `AccountSetupWizardStep` and is used to extend the default functionality of the account setup
 * process.
 *
 * If you wish to add multiple steps, they should still be handled by resolving the appropriate `AccountSetupWizardStep`
 *
...
class AdditionalAccountSetupWizardStepImpl : AdditionalAccountSetupWizardStep {
    override fun <T : Any> create(viewModel: T, step: AccountSetupWizardStep): WizardStep {
        throw IllegalArgumentException(
            "Creating an AccountSetupWizard step with ${step::class} is unsupported and requires an implementation"
        )
    }
}
        ...
        remember(viewModel, setupSteps, i18n) {
            setupSteps.map {
                when (it) {
                    ...
                    is AccountSetupWizardStep.AccessibilityStep -> wizardStepAccessibility(viewModel, it, i18n)

                    is AccountSetupWizardStep.NotificationSettingsStep -> wizardStepNotification(viewModel, it, i18n)

                    is AccountSetupWizardStep.ConfirmationStep -> wizardStepConfirmation(viewModel, it, i18n)
...
private fun wizardStepConfirmation(
    ...
    return WizardStep(
        id = step.stepId,
        title = { i18n.accountSetupWizardFinishSetupTitle() },
        content = { Text(i18n.accountSetupWizardFinishSetup()) },
        nextButton = {
            Custom {
                ThemedButton(
