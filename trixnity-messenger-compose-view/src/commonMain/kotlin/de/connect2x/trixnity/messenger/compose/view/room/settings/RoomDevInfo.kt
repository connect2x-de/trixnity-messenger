...
fun RoomDevInfoContainer(roomDevInfoViewModel: RoomDevInfoViewModel) {
    Box(Modifier.fillMaxWidth()) {
        Box(Modifier.fillMaxHeight().align(Alignment.CenterEnd)) { DevInfo(roomDevInfoViewModel) }
    }
}

interface RoomDevInfoView {
    @Composable fun create(roomDevInfoViewModel: RoomDevInfoViewModel)
}
            ...
            Box(Modifier.fillMaxSize()) {
                Column(Modifier.padding(start = 8.dp, end = 8.dp).verticalScroll(scrollState)) {
                    DevInfoCard(
                        ...
                        additionalButtons = {
                            CopyToClipboardButton(roomDevInfoViewModel.roomId.full, i18n.copyToClipboardButton())
                        },
                    ) {
                    ...
                    if (roomDevInfoViewModel.showResetEncryptionButton.collectAsState().value) {
                        HorizontalDivider()
                        ResetEncryptionButton(i18n.roomSettingsResetEncryption()) {
                            roomDevInfoViewModel.resetEncryption()
                        }
                    }
                }
...
@Composable
fun ResetEncryptionButton(contentDescription: String, onClick: () -> Unit) {
    Row(verticalAlignment = Alignment.CenterVertically) {
        ThemedButton(style = MaterialTheme.components.destructiveButton, onClick = { onClick() }) {
            Icon(
            ...
            Text(text = contentDescription, style = MaterialTheme.typography.labelLarge)
        }
    }
}
