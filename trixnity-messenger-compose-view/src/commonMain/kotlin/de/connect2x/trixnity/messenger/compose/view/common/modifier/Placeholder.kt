...
// TODO since accompanist only supports android as of now we have to copy -> in the future use lib published for compose
// jb, too
 ...
 * [de.connect2x.trixnity.messenger.compose.view.common.modifier.PlaceholderHighlight].
 */
object PlaceholderDefaults {
    /**
     * The default [InfiniteRepeatableSpec] to use for
     * [de.connect2x.trixnity.messenger.compose.view.common.modifier.fade].
     */
    val fadeAnimationSpec: InfiniteRepeatableSpec<Float> by lazy {
        infiniteRepeatable(animation = tween(delayMillis = 200, durationMillis = 600), repeatMode = RepeatMode.Reverse)
    }

    /**
     * The default [InfiniteRepeatableSpec] to use for
     * [de.connect2x.trixnity.messenger.compose.view.common.modifier.shimmer].
     */
    val shimmerAnimationSpec: InfiniteRepeatableSpec<Float> by lazy {
        infiniteRepeatable(animation = tween(durationMillis = 1700, delayMillis = 200), repeatMode = RepeatMode.Restart)
    }
}
 ...
 *
 * A version of this modifier which uses appropriate values for Material themed apps is available in the 'Placeholder
 * Material' library.
 *
 * You can provide a [PlaceholderHighlight] which runs an highlight animation on the placeholder. The
 * [de.connect2x.trixnity.messenger.compose.view.common.modifier.shimmer] and
 * [de.connect2x.trixnity.messenger.compose.view.common.modifier.fade] implementations are provided for easy usage.
 *
 * A cross-fade transition will be applied to the content and placeholder UI when the [visible] value changes. The
 * transition can be customized via the [contentFadeTransitionSpec] and [placeholderFadeTransitionSpec] parameters.
 *
 * You can find more information on the pattern at the Material Theming
 * [Placeholder UI](https://material.io/design/communication/launch-screen.html#placeholder-ui) guidelines.
 *
 ...
 * @param placeholderFadeTransitionSpec The transition spec to use when fading the placeholder on/off screen. The
 *   boolean parameter defined for the transition is [visible].
 * @param contentFadeTransitionSpec The transition spec to use when fading the content on/off screen. The boolean
 *   parameter defined for the transition is [visible].
 */
        ...
        remember(color, shape, highlight) {
            drawWithContent {
                ...
                if (contentAlpha in 0.01f..0.99f) {
                    ...
                    withLayer(paint) { with(this@drawWithContent) { drawContent() } }
...
private fun DrawScope.drawPlaceholder(
    ...
    if (shape === RectangleShape) {
        ...
        if (highlight != null) {
            drawRect(brush = highlight.brush(progress, size), alpha = highlight.alpha(progress))
        }
    ...
    if (highlight != null) {
        drawOutline(outline = outline, brush = highlight.brush(progress, size), alpha = highlight.alpha(progress))
    }
