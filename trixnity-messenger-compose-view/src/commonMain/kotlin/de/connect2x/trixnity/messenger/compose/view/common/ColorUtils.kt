...
inline val Color.saturation: Float
    get() {
        val max = maxChannel
        val delta = max - minChannel
        return (if (delta == 0F) 0F else delta / (1F - (2F * lightness - 1F).absoluteValue)).coerceIn(0F..1F)
    }

/**
 * Helper method to pick a bright or a dark color that would be readable on the current color based on its luminance.
 */
