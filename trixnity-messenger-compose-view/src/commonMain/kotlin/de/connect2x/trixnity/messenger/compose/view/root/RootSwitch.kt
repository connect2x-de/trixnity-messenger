...
fun RootSwitch(stack: Value<ChildStack<*, RootRouter.Wrapper>>) {
    Children(stack = stack, animation = stackAnimation(fade())) {
        when (val child = it.instance) {
            ...
            is RootRouter.Wrapper.OAuth2AuthorizationCodeLogin -> ConnectingWizard(child.viewModel)
            is RootRouter.Wrapper.OAuth2DeviceAuthorizationLogin -> ConnectingWizard(child.viewModel)
