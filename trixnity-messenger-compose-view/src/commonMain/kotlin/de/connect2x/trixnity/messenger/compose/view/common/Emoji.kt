...
@Composable
fun EmojiSelector(modifier: Modifier = Modifier, onTextAdded: (String) -> Unit, onDismiss: () -> Unit) {
...
@Composable
fun EmojiButton(label: String, modifier: Modifier = Modifier, onClick: () -> Unit) {
    Box(
        modifier =
            modifier
                .customClickable(
        ...
        Text(text = label, style = LocalTextStyle.current.copy(fontSize = 18.sp, textAlign = TextAlign.Center))
    }
}

/**
 * Custom emoji picker which arranges items similar to [androidx.compose.foundation.layout.Arrangement.SpaceBetween]
 * with the exception of the last line, which is left-aligned and positioned with the same spacing as previous lines
 *
 * @param modifier The modifier to use on the layout
 * @param content The emojis to arrange within the picker. For correct arrangement they are required to all have the
 *   same size
 */
@Composable
