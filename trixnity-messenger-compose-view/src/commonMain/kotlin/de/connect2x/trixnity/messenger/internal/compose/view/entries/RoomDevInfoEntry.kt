...
import de.connect2x.trixnity.messenger.compose.view.room.settings.RoomDevInfoContainer
...
internal class RoomDevInfoEntry : NavigationEntry<RoomDevInfoRoute> {

    override val metadata: Map<String, Any> = TwoPaneScene.right() + ThreePaneScene.right()

    @Composable
    override fun Content(route: RoomDevInfoRoute) {
        ThemedSurface(style = MaterialTheme.components.details) {
            RoomDevInfoContainer(roomDevInfoViewModel = rememberComponent<RoomDevInfoViewModel>())
        }
    }
}
