...
fun Modifier.expandable(isExpanded: MutableState<Boolean>) =
    this.expandable(isExpanded.value) { isExpanded.value = it }

fun Modifier.expandable(isExpanded: Boolean, setExpanded: (Boolean) -> Unit = {}) =
    this.semantics {
        if (isExpanded) collapse { setExpanded(false); true }
