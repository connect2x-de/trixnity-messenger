...
private val log: Logger =
    Logger("de.connect2x.messenger.compose.view.room.timeline.element.details.ElementDetailsViewSelectorKt")
    ...
    override fun selectFactory(element: TimelineElementViewModel<*>): TimelineElementDetailsView<TimelineElementViewModel<*>>? {
        ...
        val foundFactory = mimeTypeString?.let { mimeType ->
            try {
                ...
                factories.firstOrNull { factory ->
                    factory.supports.isInstance(element) && (factory.supportsMimeType(mimeType))
                }.also { factory ->
                    log.trace { "Found factory $factory to support details view of $element" }
                }
            } catch (e: Exception) {
