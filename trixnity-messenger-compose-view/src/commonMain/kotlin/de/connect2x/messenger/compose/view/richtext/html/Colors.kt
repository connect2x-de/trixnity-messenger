...
internal val namedColors: Map<String, Color> = mapOf(
...
private val colorRegex = Regex("^([a-z]+)\\((.*)\\)$")
private val hexShort = Regex("#([A-Fa-f0-9])([A-Fa-f0-9])([A-Fa-f0-9])([A-Fa-f0-9])?")
private val hexLong = Regex("#([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})([A-Fa-f0-9]{2})?")

// TODO: more color formats
internal fun parseColor(color: String): Color =
    namedColors[color.trim().lowercase()]
        ?: hexShort.matchEntire(color)?.let {
            val (_, r, g, b, a) = it.groupValues
            parseHexColor(r, g, b, a)
        } ?: hexLong.matchEntire(color)?.let {
            val (_, r, g, b, a) = it.groupValues
            parseHexColor(r, g, b, a)
        } ?: Color.Unspecified

internal fun parseHexColor(r: String, g: String, b: String, a: String): Color? {
    return Color(
        r.toIntOrNull(16) ?: return null,
        g.toIntOrNull(16) ?: return null,
        b.toIntOrNull(16) ?: return null,
        alpha = a.toIntOrNull(16) ?: 0xFF
    )
}
