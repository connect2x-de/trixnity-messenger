...
class ColorUtilsTest {
    ...
    fun shouldYieldBrightColor() {
        listOf(Color.Blue, Color.Black, Color.DarkGray).forEach { tintColor ->
            val contrastColor = tintColor.contrastByLuminance(brightColor, darkColor)
            assertEquals(contrastColor.value, brightColor.value)
        }
    }

    @Test
    fun shouldYieldDarkColor() {
        listOf(Color.Yellow, Color.White, Color.Cyan).forEach { tintColor ->
            val contrastColor = tintColor.contrastByLuminance(brightColor, darkColor)
            assertEquals(contrastColor.value, darkColor.value)
        }
    }
    ...
    fun shouldAssertOnIllegalArguments() {
        ...
        assertFails { tintColor.contrastByLuminance(darkColor, brightColor) }
    }
    ...
    fun shouldCalculateCorrectHexString() {
        listOf(
            ...
            .forEach { testData -> assertEquals(testData.second, testData.first.toHex()) }
    }
}
