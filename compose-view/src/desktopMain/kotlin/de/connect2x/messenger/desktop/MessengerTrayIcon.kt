...
class MessengerTrayIcon(private val unreadMessages: Long, iconSize: Float = 32f) : Painter() {
    override val intrinsicSize = Size(iconSize, iconSize)

    companion object {
        private val bitmap16 = MessengerTrayIcon::class.java.getResourceAsStream("/logo_16.png")
            ?.readAllBytes()
            ?.let { Image.makeFromEncoded(it) }
            ?.toComposeImageBitmap()
        private val bitmap32 = MessengerTrayIcon::class.java.getResourceAsStream("/logo_32.png")
            ?.readAllBytes()
            ?.let { Image.makeFromEncoded(it) }
            ?.toComposeImageBitmap()
        private val bitmap64 = MessengerTrayIcon::class.java.getResourceAsStream("/logo_64.png")
            ?.readAllBytes()
            ?.let { Image.makeFromEncoded(it) }
            ?.toComposeImageBitmap()
        private val bitmap = MessengerTrayIcon::class.java.getResourceAsStream("/logo.png")
            ?.readAllBytes()
            ?.let { Image.makeFromEncoded(it) }
            ?.toComposeImageBitmap()
    }
    ...
    override fun DrawScope.onDraw() {
        val offset = if (getOs() == OS.MAC_OS) {
            when (size.width) {
                22f, 32f -> Offset(3f, 3f)
                44f, 64f -> Offset(6f, 6f)
                else -> Offset(0f, 0f)
            }
        } else Offset(0f, 0f)
        val logoName = when {
...
private fun createImageBitmap(unreadMessages: Long, iconSize: Float): ImageBitmap {
